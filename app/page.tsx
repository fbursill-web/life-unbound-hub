...
const ShiftsChart = ({ shifts }) => {
  ...
  return (
    <svg viewBox="0 0 190 120" className="w-full max-w-[180px]">
      ...
      <text x="56"  y={H+16}                  textAnchor="middle" fontSize="9"  fill="#94a3b8" fontWeight="700" letterSpacing="1">FILLED</text>
      ...
      <text x="134" y={H+16}                  textAnchor="middle" fontSize="9"  fill="#94a3b8" fontWeight="700" letterSpacing="1">OPEN</text>
    </svg>
  );
};
...
const Modal = ({ onClose, title, wide, children }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={onClose}>
    <div className={`bg-white rounded-2xl shadow-2xl w-full ${wide ? 'max-w-3xl' : 'max-w-xl'} max-h-[92vh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
      <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-slate-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
        ...
        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-red-50 hover:text-red-500 text-slate-500 font-bold text-lg transition-colors">x</button>
      </div>
...
const Label = ({ children }) => <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{children}</label>;
const Inp = (props) => <input {...props} className={`w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all ${props.className || ''}`}/>;
const Txa = (props) => <textarea {...props} className={`w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none ${props.className || ''}`}/>;
const Sel = (props) => <select {...props} className={`w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-300 ${props.className || ''}`}/>;
...
const Btn = ({ variant = 'primary', size = 'md', className = '', ...p }) => {
  ...
  return <button {...p} className={`${base}${sizes[size]} ${variants[variant]} ${className}`}/>;
};
...
export default function LifeUnboundPortal() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 26; i++) {
      ...
      periods.push({ id: `FN-${i}`, label: `Fortnight ${i}: ${s.toLocaleDateString('en-AU',{day:'numeric',month:'short'})} - ${e.toLocaleDateString('en-AU',{day:'numeric',month:'short'})}` });
      s.setDate(s.getDate() + 14);
    }
    setFortnights(periods);
    setSelectedFN(periods[10]?.id || '');
    setAvailFN(periods[10]?.id || '');
  }, []);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error || !data)                                      { toast('No account found.', 'error'); return; }
      ...
      setUser(data); setCurrentTab('dashboard');
  ...
  const handleRegisterWorker = async (e) => {
    ...
    try {
      ...
      fetchData();
    } catch (e) { toast(e.message || 'Error creating worker.', 'error'); }
  };

  const handleRegisterParticipant = async (e) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('participants').insert([{ name: pName.trim(), ndis_number: pNdis.trim() }]);
      if (error) throw error;
      toast('Participant registered.'); setPName(''); setPNdis('');
      fetchData();
    } catch (e) { toast(e.message || 'Error registering participant.', 'error'); }
  };
  ...
  const handleCreateShift = async (e) => {
    ...
    try {
      ...
      if (allocType === 'available') {
        ...
        ws.forEach(w => ps.forEach(p => rows.push({ title, staff_id: w, participant_id: p, start_time: startIso, end_time: endIso, manager_directives: shiftDirectives.trim(), status: w ? 'scheduled' : 'available' })));
      }
      const { error } = await supabase.from('shifts').insert(rows);
      if (error) throw error;
      toast(`Published ${rows.length} shift(s).`);
      setShiftTitle(''); setShiftDirectives(''); setShiftDate(''); setShiftStart(''); setShiftEnd('');
      setSelWorkers([]); setSelParticipants([]);
      fetchData();
    } catch (e) { toast(e.message || 'Error creating shift.', 'error'); }
  };
  ...
  const handleSaveShift = async () => {
    try {
      ...
      if (error) throw error;
      toast('Shift updated.'); setEditShiftOpen(false); fetchData();
    } catch (e) { toast(e.message || 'Error updating shift.', 'error'); }
  };
  ...
  const handleDeleteShift = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast('Shift deleted.'); setEditShiftOpen(false); fetchData();
    } catch (e) { toast(e.message || 'Error deleting.', 'error'); }
  };

  const openAssign = (shift) => { setAssignTarget(shift); setAssignToId(''); setAssignOpen(true); };

  const handleAssign = async () => {
    if (!assignToId) { toast('Select a worker.', 'error'); return; }
    try {
      const { error } = await supabase.from('shifts').update({ staff_id: assignToId, status: 'scheduled' }).eq('id', assignTarget.id);
      if (error) throw error;
      toast('Shift assigned.'); setAssignOpen(false); fetchData();
    } catch (e) { toast(e.message || 'Error assigning shift.', 'error'); }
  };
  ...
  const setExtField  = (id, f, v)   => setPartDetails(prev => ({ ...prev, [id]: { ...(prev[id] || {}), [f]: v } }));
  ...
  const handleFileUpload = (id, e) => {
    ...
    Promise.all(files.map(f => new Promise(res => {
      ...
      r.readAsDataURL(f);
    }))).then(nf => setPartDetails(prev => ({ ...prev, [id]: { ...(prev[id] || {}), files: [...existing, ...nf] } })));
  };
  ...
  const getWeekDates = () => {
    ...
    const mon   = new Date(today); mon.setDate(today.getDate() + toMon + calOffset * 7);
  ...
  const calHeader = () => {
    ...
    if (calView === 'week') {
      ...
      return `${wk[0].toLocaleDateString('en-AU',{day:'numeric',month:'short'})} - ${wk[6].toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'})}`;
    }
    const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() + calOffset);
      ...
      {notification && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
          <div className={`px-5 py-3.5 rounded-xl shadow-2xl border text-xs font-bold flex items-center gap-3 ${notification.type === 'error' ? 'bg-red-50 border-red-300 text-red-700' : 'bg-green-50 border-green-300 text-green-700'}`}>
            <span className="text-base">{notification.type === 'error' ? '!' : 'ok'}</span>
            {notification.msg}
          </div>
        </div>
      )}
          ...
          <div className="max-w-lg mx-auto mt-16">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
              ...
              <div className="p-6 space-y-3">
                ...
                <button onClick={() => setPortalType('staff')}
                ...
                <button onClick={() => setPortalType('admin')}
                  ...
                  {user.role === 'director' && (
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col items-center">
                      ...
                      <div className="mt-3 w-full bg-slate-100 rounded-full h-2">
                        <div className="bg-blue-600 rounded-full h-2 transition-all" style={{ width: shifts.length ? `${(shifts.filter(s => s.staff_id).length / shifts.length) * 100}%` : '0%' }}/>
                      </div>
              ...
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2 border-b border-slate-200 pb-3">
                  {[
                    ...
                    { id: 'availability', label: `Availability Log (${availSubmissions.length})` },
                  ].map(t => (
                ...
                {adminTab === 'workers' && (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                      ...
                      {workers.length === 0 ? <p className="text-xs text-slate-300 text-center py-6">No staff registered yet.</p> : (
                ...
                {adminTab === 'participants' && (
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 max-w-lg space-y-4">
                    ...
                    <p className="text-[9px] text-slate-400">After registering, visit the Participants tab to add full medical and contact details.</p>
                  </div>
                )}
                ...
                {adminTab === 'timesheets' && (
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                      ...
                      <span className="text-[10px] text-slate-400 font-bold">{timesheetHistory.length} total submissions</span>
                    </div>
                        ...
                        {timesheetHistory.map(ts => (
                          <div key={ts.id} className="px-5 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                            <div>
                              ...
                              <span className="block text-[10px] text-slate-400">{ts.rows?.length || 0} shifts &middot; {ts.totalHours} hrs &middot; Submitted {ts.submittedAt}</span>
                            </div>
                            <Btn variant="secondary" size="sm" onClick={() => { setViewingTs(ts); setViewTsOpen(true); }}>View Details</Btn>
                          </div>
                        ))}
                      </div>
                    )}
                          ...
                          <div key={i} className="px-5 py-4">
                            <div className="flex items-center justify-between mb-3">
                              <div>
                                ...
                                <span className="block text-[10px] text-slate-400">{sub.fortnightLabel} &middot; Submitted {sub.submittedAt}</span>
                              </div>
                            </div>
            ...
            {currentTab === 'rosters' && (
              <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
                    {['day','week','month'].map(v => (
                      <button key={v} onClick={() => { setCalView(v); setCalOffset(0); }}
                        className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${calView === v ? 'bg-blue-600 text-white shadow' : 'text-slate-500 hover:text-slate-800'}`}>{v}</button>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5">
                        {['shift','event'].map(c => (
                          <button key={c} type="button" onClick={() => setEvtCategory(c)}
                            className={`px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-wider transition-all ${evtCategory === c ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>{c}</button>
                        ))}
                      </div>
                    </div>
                    <form onSubmit={handleCreateShift}>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div className={allocType === 'staff' || allocType === 'participant' ? 'sm:col-span-2' : 'sm:col-span-3'}>
                          ...
                          <Inp value={shiftDirectives} onChange={e => setShiftDirectives(e.target.value)} placeholder="e.g. Bring medication logbook, call supervisor on arrival"/>
                        </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      <div className="p-4">
                        ...
                        <div className="grid grid-cols-7 gap-1">
                          {Array.from({ length: gridCells }).map((_, i) => {
                            ...
                            return (
                              <div key={i} className={`rounded-xl border min-h-[80px] p-1.5 flex flex-col ${valid ? 'bg-white border-slate-200 hover:border-blue-300 transition-colors' : 'bg-slate-50/50 border-slate-100'} ${isToday ? 'ring-2 ring-blue-400 border-blue-300' : ''}`}>
                                {valid && (
                                  <>
                                    ...
                                    <div className="space-y-0.5 flex-1 overflow-hidden">
                                      {dShifts.slice(0, 3).map(s => (
                                        <button key={s.id} onClick={() => openEditShift(s)}
                  ...
                  {calView === 'day' && (() => {
                    const target = new Date(); target.setDate(target.getDate() + calOffset);
                        ...
                        {hours.map(h => {
                          ...
                          return (
                            <div key={h} className="py-3 flex gap-4 min-h-[52px]">
                              <span className="w-14 text-right text-[10px] font-bold text-blue-500 font-mono shrink-0 pt-0.5">{h.toString().padStart(2,'0')}:00</span>
            ...
            {currentTab === 'profiles' && (
              <div className="space-y-5">
                ...
                {participants.length === 0 && (
                  <div className="bg-white border border-slate-200 rounded-xl p-12 text-center text-slate-300 text-xs">No participants registered yet. Add them via Admin Centre.</div>
                )}
                  ...
                  {participants.map(p => {
                    ...
                    return (
                      <div key={p.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                        <div onClick={() => setExpandedPartId(isOpen ? null : p.id)}
                          className="p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-colors select-none">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center text-blue-700 font-black text-sm shrink-0">{p.name?.charAt(0) || '?'}</div>
                            <div>
                          ...
                          <div className="flex items-center gap-2">
                            {ext.medicalConditions && <span className="text-[9px] bg-red-50 border border-red-200 text-red-600 font-bold px-2 py-0.5 rounded-md uppercase hidden sm:block">Medical Info</span>}
                          ...
                          <div className="border-t border-slate-100 p-5 space-y-5 bg-slate-50/30">
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                              {[
                                ...
                                ['GP',            ext.gpName || 'Not set'],
                              ].map(([lbl, val]) => (
                                <div key={lbl}>
                                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-0.5">{lbl}</p>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                  {ext.files.map((f, i) => (
                                    <a key={i} href={f.data} download={f.name}
                                      className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl p-2.5 hover:bg-blue-50 hover:border-blue-300 transition-colors text-xs font-semibold text-slate-700 truncate">
                                      <span>{f.type?.includes('pdf') ? 'PDF' : f.type?.startsWith('image') ? 'IMG' : 'DOC'}</span>
                        ...
                        {isOpen && isEdit && (
                          <div className="border-t border-slate-100 p-5 space-y-5 bg-blue-50/20">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div><Label>NDIS Goals</Label><Txa rows={4} value={ext.ndisGoals || ''} onChange={e => setExtField(p.id, 'ndisGoals', e.target.value)} placeholder="Participant goals as per NDIS plan..."/></div>
                            ...
                            <div>
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <Btn variant="secondary" size="sm" type="button" onClick={() => setExtField(p.id, 'emergencyContacts', [...(ext.emergencyContacts || []), { name:'', relationship:'', phone:'' }])}>+ Add Contact</Btn>
                              </div>
                              <div className="space-y-2">
                                {(ext.emergencyContacts || []).map((ec, i) => (
                                  <div key={i} className="grid grid-cols-3 gap-2 items-center bg-white border border-slate-200 rounded-xl p-3">
                                    ...
                                    <div className="flex gap-2">
                                      ...
                                      <button type="button" onClick={() => { const c = [...(ext.emergencyContacts || [])]; c.splice(i, 1); setExtField(p.id, 'emergencyContacts', c); }} className="text-red-400 hover:text-red-600 font-black text-lg shrink-0">x</button>
                                    </div>
                                  </div>
                                ))}
                              </div>
                            ...
                            <div>
                              ...
                              <div className="border-2 border-dashed border-slate-300 hover:border-blue-400 rounded-xl p-6 text-center transition-colors cursor-pointer bg-white"
                                ...
                                <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                                  {ext.files.map((f, i) => (
                                    <div key={i} className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-2.5 gap-2">
                                      ...
                                      <button type="button" onClick={() => removeFile(p.id, i)} className="text-red-400 hover:text-red-600 shrink-0 font-black">x</button>
                                    </div>
                                  ))}
                                </div>
                              )}
                      ...
                      return (
                        <div key={day} className={`p-4 flex flex-col sm:flex-row sm:items-center gap-3 transition-colors ${st.mode === 'unavailable' ? 'bg-red-50/40' : st.mode === 'allday' ? 'bg-green-50/40' : 'bg-white'}`}>
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {[['standard','Hours'],['allday','All Day'],['unavailable','Unavailable']].map(([m, l]) => (
                              <button key={m} type="button" onClick={() => setAvailMode(day, m)}
                        ...
                        {tsRows.map((row, i) => (
                          <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2 lg:space-y-0 lg:grid lg:grid-cols-12 lg:gap-2 lg:items-start shadow-sm">
                            ...
                            <div className="lg:col-span-1 flex items-start justify-end">
                              {tsRows.length > 1 && <button type="button" onClick={() => removeTsRow(i)} className="text-red-400 hover:text-red-600 font-black text-lg mt-0.5">x</button>}
                            </div>
                            <div className="lg:col-span-12">
                              <Inp required value={row.notes} onChange={e => updateTs(i,'notes',e.target.value)} placeholder="Activity notes - what did you support the participant with?"/>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {timesheetHistory.filter(t => t.workerId === user.id).length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-80 overflow-y-auto">
                        {timesheetHistory.filter(t => t.workerId === user.id).map(ts => (
                          <div key={ts.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer" onClick={() => { setViewingTs(ts); setViewTsOpen(true); }}>
                            <p className="text-xs font-bold text-slate-700">{ts.fortnightLabel}</p>
                            <p className="text-[10px] text-slate-400">{ts.rows?.length || 0} shifts &middot; {ts.totalHours} hrs</p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                ['Total Hrs', viewingTs.totalHours],
                ['Submitted', viewingTs.submittedAt],
              ].map(([l,v]) => (
                <div key={l} className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center">
                  <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider">{l}</p>
                  <p className="text-sm font-black text-slate-800 mt-0.5 truncate">{v}</p>
                </div>
              ))}
            </div>
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              <div className="bg-slate-50 px-4 py-2.5 grid grid-cols-12 gap-2 text-[8px] font-black uppercase text-slate-400 tracking-widest border-b border-slate-200">
                <div className="col-span-2">Date</div><div className="col-span-1">Start</div><div className="col-span-1">End</div>
                <div className="col-span-2">Client</div><div className="col-span-1">KM+c</div><div className="col-span-1">KM</div>
